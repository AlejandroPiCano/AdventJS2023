...
function distributeGifts(weights) {
    let result = JSON.parse(JSON.stringify(weights));
    ...
    for (let i = 0; i < weights.length; i++) {
        for (let j = 0; j < weights[i].length; j++) {
            ...
            if (j - 1 >= 0 && weights[i][j - 1] != null) {
                sum += weights[i][j - 1];
                count++
            };
            if (j + 1 < weights[i].length && weights[i][j + 1] != null) {
                sum += weights[i][j + 1];
                count++
            };
            if (i - 1 >= 0 && weights[i - 1][j] != null) {
                sum += weights[i - 1][j];
                count++
            };
            if (i + 1 < weights.length && weights[i + 1][j] != null) {
                sum += weights[i + 1][j];
                count++
            };
            if (weights[i][j] != null) {
