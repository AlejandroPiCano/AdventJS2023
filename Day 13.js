...
function calculateTime(deliveries) {
    ...
    for (let delivery of deliveries) {
        let [hours, minutes, seconds] = delivery.split(':')
        totalSeconds += parseInt(hours) * 3600
        totalSeconds += parseInt(minutes) * 60
        totalSeconds += parseInt(seconds);
    }

    let topInSeconds = 7 * 3600;
    let diff = Math.abs(topInSeconds - totalSeconds);
    let operator = topInSeconds > totalSeconds ? "-" : ""

    let hours = Math.floor(diff / 3600);
    let minutes = Math.floor((diff - hours * 3600) / 60);
    let seconds = diff - hours * 3600 - minutes * 60;
    let result = `${operator}${hours<10?`0${hours}`:hours}`;
