...
function checkIsValidCopy(original, copy) {
    ...
    for (let i = 0; i < original.length; i++) {
        if (copy[i] != original[i]) {
            if (!isNaN(original[i]) || !isNaN(original[i]))
                return false;

            if (original[i].toLowerCase() != copy[i]) {
                if (symbols.indexOf(copy[i]) == -1)
                    return false;
                else {
                    let indCopy = symbols.indexOf(copy[i]);
                    let indOriginal = symbols.indexOf(original[i]);
                    if (indCopy < indOriginal)
                        return false;
                }
            }
        }
