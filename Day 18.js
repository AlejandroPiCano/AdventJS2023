...
function drawClock(time) {
    ...
    let result = digits[parseInt(time[0])];
    result = concatMatrix(result, space);

    for (let i = 1; i < time.length; i++) {
        if (time[i] == ":")
            result = concatMatrix(result, separator);
        else
            result = concatMatrix(result, digits[parseInt(time[i])]);
