...
function transformTree(tree) {
    function transformTreeRec(tree, index) {
        if (index >= tree.length || tree[index] == null)
            return null;

        let obj = {};
        obj.value = tree[index];
        obj.left = transformTreeRec(tree, 2 * index + 1);
        obj.right = transformTreeRec(tree, 2 * index + 2);
        return obj;
    }

    let result = transformTreeRec(tree, 0);
